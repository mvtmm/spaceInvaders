...
class IShip:
    ...
    def drawHealthBar(self):
        ...
        innerrect = pygame.Rect(self.game.playerX, self.game.playerY + 10,
                                self.game.playerHealth / 100 * self.playerShipRectSize[1], 5)
        # Rand um den Füllstand berechnen unterhalb des Spielerschiffes
        border_rect = pygame.Rect(
            self.game.playerX, self.game.playerY + 10, self.playerShipRectSize[1], 5)
        # Zeichnen des Lebensbalkens
        pygame.draw.rect(self.game.game.screen,
                         ColorType.Green.value, innerrect)
        # Zeichnen des Lebensbalkens Rand
        pygame.draw.rect(self.game.game.screen,
                         ColorType.White.value, border_rect, 2)
    ...
    def drawArmorBar(self):
        ...
        for x in range(self.game.playerShield):
            ...
            schildRect = pygame.Rect(
                self.game.playerX + (x * 20), self.game.playerY - 30, 19, 5)
            # Schild zeichnen
    ...
    def moveRect(self):
        ...
        self.playerShipRect = pygame.Rect(
            self.game.playerX, self.game.playerY - 80, 64, 64)
...
class Ship_One(IShip):
    ...
    def draw(self):
        ...
        if self.playership is None:
            self.playership = Assetloader.getAsset(
                AssetType.Graphics, "Ship1.png")
...
class Ship_Five(IShip):
    ...
    def draw(self):
        ...
        if self.playership is None:
            self.playership = Assetloader.getAsset(
                AssetType.Graphics, "Ship5.png")
            # Spielerschiff um 90 ° drehen
            self.playership = pygame.transform.rotate(self.playership, 90)
            # Spielerschiff skalieren
            self.playership = pygame.transform.scale(
                self.playership, (100, 100))
        # Spielerschiff bewegen & zeichnen
        self.moveRect()
